...
## 12) Plan 이관 요약 (2026-04-25)

`plan.md`를 진행 문서로 축약하면서, 기존 0~14번 완료 내역은 아래 핵심만 보존해 이관함.

### 완료 항목(0~14) 핵심
- 0: ZA 메가진화 공개 특성/종족값 최신화(`Starmie-Mega` 등 반영).
- 1: 온라인 준비 취소 후 빌더 수정 미반영 문제를 자동 동기화(debounce/signature)로 해결.
- 2: 모바일 다이맥스 입력 시 포커스가 토글로 강제 복귀되던 문제 수정.
- 3: 배틀 종료 승리 메시지의 조기 출력 시점을 턴 연출 이후로 조정.
- 4: 종료 시 날씨/지형/함정/진영효과/지형 배경 초기화.
- 5: 팀 슬롯 드래그 정렬 구현.
- 6: 포켓몬 검색 엔진 강화(타입 필터, BST/스탯 정렬, species 메타 인덱스).
- 7: 길동무 후 비정상 `Struggle` 강제 흐름 수정(forceSwitch 우선, Struggle 자동강제 제외).
- 8: 아이템 출처 damage/heal 메시지(먹밥/생구 등) 추가.
- 10: 시작/필드 전개 구간 렉 완화(렌더 동기화 중복 완화, 지형 BG 선로드, pre-hide 연계).
- 11: 시작 직전 스프라이트 선노출 문제 해결(`switch_in(fromBall)` 사이드 pre-hide).
- 12: `cant_move` 이유(마비/풀죽음/잠/얼음/재충전 등) 문구 분기.
- 13: 다이맥스/Z 연출 및 위력 표시 보강(Max Guard=Protect, 타입별 Max 연출 매핑, UI 위력 보정).
- 14: 온라인 빨간 경고 메시지를 자기 포켓몬 기준으로만 노출(side/tone 필터).

### 안정화 보강 핵심
- 애니메이션 프레임/텍스처 안정성 보강:
  - 잘못된 프레임 요청 fallback(`PRAS- Torment`류 경고 대응)
  - 프레임 루프 예외 시 cleanup 강제
  - 텍스처 해제를 지연/안전 검사 기반으로 처리(`glTexture null` 계열 크래시 완화)
- 섀도볼 스프라이트 소실 원인 정정:
  - USER/TARGET 복제 스프라이트에 `priority`를 적용하던 포팅 오차 제거.
  - 원본 규칙처럼 `priority` 재배치는 GRAPHIC 스프라이트에만 적용.
