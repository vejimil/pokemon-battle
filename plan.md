...
## 2026-04-29 더블배틀 빠른 연출 1차 구현
- 사용자 각주 반영 문서:
  - `docs/double-battle-fast-timeline-plan.md`
- 구현 범위:
...
- 브라우저 수동 확인 포인트:
  - 더블 시작 시 P1 두 마리 등장 후 P2 두 마리 등장으로 묶이고, 각 묶음 안의 볼 연출이 동시에 재생되는지 확인.
  - 지진/스톤샤워/방전/열풍 등 광역기 피해 대상들의 HP바가 동시에 줄어드는지 확인.
  - 모래바람/싸라기눈 피해 대상들의 HP바가 동시에 줄고, 메시지가 대상명을 묶어 표시되는지 확인.
  - 기절/faint 연출, Commander 복원, party UI 복귀가 기존처럼 정상인지 확인.
  - 싱글 배틀에서는 기존 순차 연출과 상태 선반영 방지 동작이 유지되는지 확인.
...
8. **DB-8 forceSwitch 듀얼 슬롯** ✅ 완료(2026-04-26):
   ...
   - `src/pokerogue-transplant-runtime/ui/handlers/party-ui-handler.js`:
     - party overlay hide/restore를 재진입 안전하게 보강(`captured` 플래그).
       최초 1회만 원래 visible 상태를 캡처하고, 이후 party 모드의 반복 render에서도
       숨김은 매번 재적용하도록 정리(강제교체 중 정보창 오버레이/복귀 후 스프라이트 소실 동시 방지).
8.5 **DB-8.5 더블 특수 로직 연출 강화** ✅ 완료(2026-04-26):
    - **구현 내용**:
      1. **엔진 이벤트 파싱** (`server/showdown-engine.cjs`):
         - `normalizeEventsFromLine`에 `swap` 케이스 추가 → `position_swap` 구조 이벤트 발행.
           `{type:'position_swap', side, fromSlot, toSlot}` 형태로 표준화.
         - 이전에는 `raw_event`로 흘러서 타임라인에서 무시됨.
      2. **초기 배치 순서 버그 수정** (`src/app.js`):
         - `getBattleActiveIndices()`의 `requestActive` 정렬 버그 수정:
           이전에는 team-index 순(낮은 팀 번호 = LEFT) 고정이었지만,
           이제 `engineOrderIndex`(p1a=0, p1b=1)를 기준으로 정렬해
           엔진이 정한 위치 순서(선출 의도)가 초기 렌더에서도 유지됨.
      3. **씬 스왑 메서드** (`src/pokerogue-transplant-runtime/scene/battle-shell-scene.js`):
         - `swapBattlerPositions(side, slot0, slot1)` 신설:
           두 슬롯의 `currentUrl`을 교환하고 `setBattlerSprite`를 병렬 호출로
           스프라이트를 즉시 교체(브라우저 캐시 활용으로 빠름).
      4. **타임라인 핸들러** (`src/battle-presentation/timeline.js`):
         - `position_swap` 케이스 추가:
