...
| 번호 | 항목 | 상태 | 메모 |
|---|---|---|---|
| 9 | 더블배틀 구현 | 진행 중(DB-1~DB-8 완료) | 아래 §9 단계별 구현 계획 참조. 다음: DB-8.5(더블 특수 로직 연출 강화). |
...
8.5 **DB-8.5 더블 특수 로직 연출 강화**:
    ...
    - 범위:
      ...
      - 사이드체인지 연계 리드 배치 보존:
        - 재현 이슈: 로비/선출 단계에서 `왼쪽=슬롯3`, `오른쪽=슬롯2`로 배치했어도,
          전투 씬 진입 후 `슬롯 번호 순`으로 강제 재정렬되어 좌/우 위치가 바뀌어 보임.
          사이드 체인지 사용 시 스프라이트 위치도 바뀌지 않고, 원래 단일 공격 위치가 바뀌지도 않으나(적 에이스번과 싸리용이 있을 때 물의파동을 에이스번에게 누르고 에이스번이 먼저 사이드 체인지를 사용해도 물의파동이 에이스번에게 나감.), 이후에 행동 선택 순서는 바뀜.
        - 원인 분석: 현재 전투 진입 시점의 active 슬롯 매핑이 `slot index`(a/b 순서) 우선으로
          재구성되며, 선출 UI의 좌/우 의도 배치 정보가 별도 키로 고정 저장되지 않아
          초기 렌더 단계에서 정규화(슬롯순)로 덮어써짐.
        - 개선 목표: 사이드체인지/초기 배치 모두에서 "표시 좌표 매핑"과 "엔진 슬롯 index"를
          분리 추적하여, 타깃팅/피격/정보창 기준 슬롯은 정확히 유지하면서도 좌/우 표시 의도를 보존.
    - 완료 기준:
      - Side Change 후 타깃팅/피격 연출 기준 슬롯이 양 클라이언트에서 동일.
      - 선출에서 의도한 좌/우 배치(예: 슬롯3-left, 슬롯2-right)가 전투 시작 직후에도 유지.
