// Generated from smogon/pokemon-showdown rulesets.ts
...
export const Rulesets = {
    ...
    forcemonotype: {
        ...
        onValidateRule(value) {
            ...
            if ((['Dark', 'Steel'].includes(type.name) && this.dex.gen < 2) ||
    ...
    accuracytrapclause: {
        ...
        onValidateSet(set, format, setHas, teamHas) {
            ...
            const accuracy = this.dex.moves.all().filter(move => {
                if (move.boosts?.accuracy)
                    return move.boosts.accuracy < 0;
                return move.secondaries?.some(x => x.chance === 100 && x.boosts?.accuracy && x.boosts.accuracy < 0);
            }).map(x => x.id);
            if (set.moves.map(this.toID).some(x => accuracy.includes(x)) && (trapping.includes(this.toID(set.ability)) || set.moves.map(this.toID).some(x => trapping.includes(x)))) {
                return [
    ...
    batonpassstatclause: {
        ...
        onValidateTeam(team) {
            ...
            for (const set of team) {
                ...
                const item = this.toID(set.item);
                const ability = this.toID(set.ability);
                if (moves.some(m => boostingEffects.includes(m)) || boostingEffects.includes(item) ||
        ...
        onValidateTeam(team, format, teamHas) {
            ...
            for (const set of team) {
                if (!set.moves.map(this.toID).includes('batonpass'))
                    continue;
                if (!set.moves.some(move => trappingMoves.includes(this.toID(move))))
                    continue;
    ...
    batonpassstattrapclause: {
        ...
        onValidateTeam(team) {
            ...
            for (const set of team) {
                ...
                const ability = this.toID(set.ability);
                for (const move of set.moves) {
                    if (statBoostOrTrapping.includes(this.toID(move)))
                        passableBoosts = true;
                }
                if (statBoostOrTrapping.includes(item))
                    passableBoosts = true;
                if (statBoostOrTrapping.includes(ability))
                    passableBoosts = true;
                if (passableBoosts) {
                    return [
    ...
    samecolorclause: {
        ...
        onValidateTeam(team) {
            ...
            for (const [i, set] of team.entries()) {
                let species = this.dex.species.get(set.species);
                if (!species.color)
                    return [`Invalid Pok\u00e9mon ${set.name || set.species}`];
                        ...
                        if (baseSpecies.otherFormes) {
                            for (const formeName of baseSpecies.otherFormes) {
                                if (baseSpecies.prevo) {
                                    ...
                                    if (prevo.evos.includes(formeName))
                                        continue;
                                }
        ...
        onValidateTeam(team) {
            ...
            for (const set of team) {
                ...
                if (species.name !== species.baseSpecies) {
                    ...
                    if (species.types.join('/') === baseSpecies.types.join('/') &&
    ...
    chimera1v1rule: {
        ...
        onValidateRule() {
            ...
            if ((table.pickedTeamSize || table.minTeamSize) < 6) {
                throw new Error(`Custom rules that could allow the active team size to be reduced below 6 (Min Team Size < 6, Picked Team Size < 6) could prevent the Chimera from being fully defined, and are incompatible with Chimera 1v1.`);
            }
            const gameType = this.format.gameType;
            if (gameType === 'doubles' || gameType === 'triples') {
                throw new Error(`The game type '${gameType}' cannot be 1v1 because sides can have multiple active Pok\u00e9mon, so it is incompatible with Chimera 1v1.`);
            }
        },
    ...
    tiershiftmod: {
        ...
        onModifySpecies(species, target, source, effect) {
            ...
            if (target && !isNatDex) {
                if (this.toID(target.set.item) === 'lightclay' && tiers.indexOf(tier) > tiers.indexOf('rubl'))
                    tier = 'rubl';
                if (this.toID(target.set.item) === 'quickclaw' && tiers.indexOf(tier) > tiers.indexOf('nubl'))
                    tier = 'nubl';
                if (this.toID(target.set.ability) === 'drought' && tiers.indexOf(tier) > tiers.indexOf('nubl'))
                    tier = 'nubl';
                if (this.toID(target.set.item) === 'damprock' && tiers.indexOf(tier) > tiers.indexOf('publ'))
                    tier = 'publ';
                if (this.toID(target.set.item) === 'unburden' && tiers.indexOf(tier) > tiers.indexOf('zubl'))
                    tier = 'zubl';
            }
    ...
    reevolutionmod: {
        ...
        onModifySpecies(species, target) {
            const newSpecies = this.dex.deepClone(species);
            const baseSpecies = this.dex.species.get((Array.isArray(species.battleOnly) ? species.battleOnly[0] : species.battleOnly) || species.changesFrom || species.name);
            if (!newSpecies.prevo) {
                if (!baseSpecies.prevo)
                    return;
    ...
    godlygiftmod: {
        ...
        onValidateTeam(team) {
            ...
            for (const set of team) {
                ...
                if ((species.baseSpecies === 'Zamazenta' && this.toID(set.item) === 'rustedshield') ||
    ...
    convergencelegality: {
        ...
        onValidateSet(set, format) {
            ...
            const obtainableAbilityPool = new Set();
            const matchingSpecies = this.dex.species.all()
                .filter(species => ((!species.isNonstandard || this.ruleTable.has(`+pokemontag:${this.toID(species.isNonstandard)}`)) &&
        ...
        checkCanLearn(move, species, setSources, set) {
            const matchingSpecies = this.dex.species.all()
                .filter(s => ((!s.isNonstandard || this.ruleTable.has(`+pokemontag:${this.toID(s.isNonstandard)}`)) &&
    ...
    hackmonsformelegality: {
        ...
        onChangeSet(set, format, setHas, teamHas) {
            ...
            const problemPokemon = this.dex.species.all().filter(s => ((s.name === 'Xerneas' || s.battleOnly || s.forme === 'Eternamax') &&
            ...
            if (problemPokemon.includes(species)) {
                ...
                set.species = (species.id === 'xerneas' ? 'Xerneas-Neutral' :
                    species.id === 'zygardecomplete' ? 'Zygarde' : species.battleOnly);
    ...
    uniquepokemoves: {
        ...
        onValidateTeam(team, format, teamHas) {
            ...
            for (const [moveid, num] of pokemoves) {
                if (num <= uniquePokemoves)
                    continue;
                problems.push(`You have ${num} Pok\u00e9mon with ${this.dex.species.get(moveid).name} as a Pokemove.`, `(Each Pok\u00e9mon can only be used as a Pokemove ${uniquePokemoves} time${uniquePokemoves === 1 ? '' : 's'} per team.)`);
            }
            return problems;
        },
    },
    ferventimpersonationmod: {
        ...
        onValidateTeam(team, format, teamHas) {
            const exhaustedSpecies = new Set();
            for (const set of team) {
                const species = this.dex.species.get(set.species);
    ...
    rebalancelevels: {
        ...
        onBegin() {
            const rebalanceLevel = (oldSpecies, set, newSpecies) => {
                ...
                let newStats = this.spreadModify(newSpecies.baseStats, set);
                ...
                statRatioTotal += statRatios.bulk = (Math.log(oldStats.hp * oldStats.def * oldStats.spd / (oldStats.def + oldStats.spd)) -
                ...
                while (newLevel !== oldLevel) {
                    ...
                    statRatioTotal += statRatios.bulk = (Math.log(oldStats.hp * oldStats.def * oldStats.spd / (oldStats.def + oldStats.spd)) -
                        Math.log(newStats.hp * newStats.def * newStats.spd / (newStats.def + newStats.spd)));
                    statRatioTotal += statRatios.speed = Math.log(oldStats.spe / newStats.spe) / 2;
                    if (overestimate && statRatioTotal >= 0 || !overestimate && statRatioTotal <= 0)
                        break;
            ...
            for (const poke of this.getAllPokemon()) {
                ...
                poke.set.level = poke.level = rebalanceLevel(oldSpecies, poke.set, newSpecies);
                ...
                if (poke.species.maxHP)
                    stats.hp = poke.species.maxHP;
        ...
        onValidateRule() {
            if (!this.format.team)
                throw new Error('The Rebalance Levels rule is only intended to work with randomized teams.');
            ...
            if (!speciesMods.length)
                throw new Error('This format has no rules that modify base stats.');
        },
    },
};
