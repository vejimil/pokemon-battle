...
class RuleTable extends Map {
    ...
    isBannedSpecies(species) {
        ...
        if (this.has(`-pokemon:${species.id}`))
            return true;
        if (this.has(`+basepokemon:${(0, dex_data_1.toID)(species.baseSpecies)}`))
            return false;
        if (this.has(`-basepokemon:${(0, dex_data_1.toID)(species.baseSpecies)}`))
            return true;
        for (const tagid in tags_1.Tags) {
    ...
    getComplexBanIndex(complexBans, rule) {
        const ruleId = (0, dex_data_1.toID)(rule);
...
class Format extends dex_data_1.BasicEffect {
    constructor(data) {
        super(data);
        this.mod = lib_1.Utils.getString(data.mod) || DEFAULT_MOD;
...
class DexFormats {
    ...
    load() {
        ...
        this.formatsListCache = this.buildList(formats_1.Formats);
        return this;
    }
    extend(formats) {
        this.load();
    ...
    buildList(formats) {
        ...
        for (const [i, f] of formats.entries()) {
            ...
            const id = (0, dex_data_1.toID)(format.name);
            if (format.section)
    ...
    validate(name) {
        ...
        const customRules = customRulesString.split(',').map(rule => {
            ...
            if (typeof ruleSpec === 'string') {
                if (ruleSpec === '-pokemontag:allpokemon' || ruleSpec === '+pokemontag:allpokemon') {
                    if (hasPokemonRule)
                        throw new Error(`You can't ban/unban pokemon before banning/unbanning all Pokemon.`);
                }
                if (this.isPokemonRule(ruleSpec))
                    hasPokemonRule = true;
            }
        ...
        if (!hasCustomRules)
            throw new Error(`None of your custom rules change anything`);
    ...
    isPokemonRule(ruleSpec) {
        return (ruleSpec.slice(1).startsWith('pokemontag:') || ruleSpec.slice(1).startsWith('pokemon:') ||
    ...
    getRuleTable(format, depth = 1, repeals) {
        ...
        for (let ruleSpec of ruleSpecs) {
            ...
            if (ruleSpec.startsWith('^'))
                ruleSpec = ruleSpec.slice(1);
            if (ruleSpec.startsWith('!') && !ruleSpec.startsWith('!!')) {
                repeals || (repeals = new Map());
        ...
        for (let ruleSpec of ruleSpecs) {
            ...
            if (ruleSpec.startsWith('!') && !ruleSpec.startsWith('!!')) {
                ...
                if (repealDepth === undefined)
                    throw new Error(`Multiple "${ruleSpec}" rules in ${format.name}`);
            ...
            if (subRuleTable.onChooseTeam) {
                if (ruleTable.onChooseTeam) {
                    throw new Error(`"${format.name}" has conflicting team selection rules from ` +
    ...
    validateRule(rule, format = null) {
        ...
        switch (rule.charAt(0)) {
            ...
            default:
                ...
                let id = (0, dex_data_1.toID)(ruleName);
    ...
    validPokemonTag(tagid) {
        const tag = tags_1.Tags.hasOwnProperty(tagid) && tags_1.Tags[tagid];
        if (!tag)
            return false;
        return !!(tag.speciesFilter || tag.genericFilter);
    }
    validateBanRule(rule) {
        let id = (0, dex_data_1.toID)(rule);
        if (id === 'unreleased')
            return 'unreleased';
        if (id === 'nonexistent')
            return 'nonexistent';
