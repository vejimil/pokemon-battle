...
class Side {
    ...
    addPokemon(set) {
        if (this.pokemon.length >= 24)
            return null;
        const newPokemon = new pokemon_1.Pokemon(set, this);
    ...
    chooseMove(moveText, targetLoc = 0, event = '') {
        ...
        if (autoChoose) {
            for (const [i, move] of request.moves.entries()) {
                if (move.disabled)
                    continue;
                if (i < moves.length && move.id === moves[i].id && moves[i].disabled)
                    continue;
                moveid = move.id;
        ...
        if (lockedMove) {
            ...
            const lockedMoveID = (0, dex_1.toID)(lockedMove);
    ...
    choose(input) {
        ...
        for (const choiceString of choiceStrings) {
            let [choiceType, data] = utils_1.Utils.splitFirst(choiceString.trim(), ' ');
            data = data.trim();
