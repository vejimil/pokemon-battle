...
class ExhaustiveRunner {
        ...
        try {
            ...
            do {
                ...
                try {
                    ...
                    this.failures++;
                    console.error(`Run \`${this.cmd(this.cycles, this.format, seed)}\`:\n`, err);
                }
    ...
    createPools(dex) {
        return {
            pokemon: new Pool(this.possible?.pokemon ??
                ExhaustiveRunner.onlyValid(dex.gen, dex.data.Pokedex, p => dex.species.get(p), (_, p) => (p.name !== 'Pichu-Spiky-eared' && p.name.substr(0, 8) !== 'Pikachu-') &&
            ...
            moves: new Pool(this.possible?.moves ??
                ExhaustiveRunner.onlyValid(dex.gen, dex.data.Moves, m => dex.moves.get(m), m => (!['struggle', 'revivalblessing'].includes(m) &&
    ...
    static getSignatures(dex, pools) {
        ...
        for (const id of pools.items.possible) {
            ...
            if (item.megaStone) {
                const pokemon = (0, dex_1.toID)(Object.keys(item.megaStone)[0]);
                const combo = { item: id };
...
class TeamGenerator {
    constructor(dex, prng, pools, signatures) {
        this.dex = dex;
        this.prng = prng_1.PRNG.get(prng);
        this.pools = pools;
