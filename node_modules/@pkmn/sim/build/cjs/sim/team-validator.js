            ...
            if (minEggGen) {
                for (const eggMoveAndGen of this.possiblyLimitedEggMoves) {
                    ...
                    if (parseInt(eggMoveAndGen.charAt(0)) < minEggGen) {
                        const eggMove = (0, dex_1.toID)(eggMoveAndGen.substr(1));
...
class TeamValidator {
    constructor(format, dex = dex_1.Dex) {
    ...
    baseValidateTeam(team, options = {}) {
        ...
        if (format.team) {
            ...
            const testTeamSeed = prng_1.PRNG.generateSeed();
            try {
                const testTeamGenerator = teams_1.Teams.getGenerator(format, testTeamSeed);
    ...
    getValidationSpecies(set) {
        ...
        if (ability.id === 'owntempo' && (0, dex_1.toID)(species.baseSpecies) === 'rockruff') {
    ...
    validateSet(set, teamHas) {
        ...
        let item = dex.items.get(utils_1.Utils.getString(set.item));
        set.item = item.name;
        let ability = dex.abilities.get(utils_1.Utils.getString(set.ability));
        set.ability = ability.name;
        let nature = dex.natures.get(utils_1.Utils.getString(set.nature));
        ...
        for (const moveName of set.moves) {
            if (!moveName)
                continue;
            const move = dex.moves.get(utils_1.Utils.getString(moveName));
            if (!move.exists)
        ...
        if ((!setSources.sourcesBefore && setSources.sources.length) || isUnderleveled) {
            ...
            if (legalSources.length) {
                ...
                if (firstEventSource) {
                    const eventProblems = this.validateSource(set, firstEventSource, setSources, outOfBattleSpecies, ` to be underleveled`);
                    if (eventProblems)
    ...
    validateStats(set, species, setSources, pokemonGoProblems) {
        ...
        for (const stat in set.evs) {
            if (set.evs[stat] < 0) {
                problems.push(`${name} has less than 0 ${allowAVs ? 'Awakening Values' : 'EVs'} in ${dex_1.Dex.stats.names[stat]}.`);
            }
        }
        if (dex.currentMod === 'gen7letsgo') { // AVs
            for (const stat in set.evs) {
                if (set.evs[stat] > 0 && !allowAVs) {
                    ...
                    problems.push(`${name} has more than 200 Awakening Values in ${dex_1.Dex.stats.names[stat]}.`);
                }
            }
        }
        else { // EVs
            for (const stat in set.evs) {
                if (set.evs[stat] > 255) {
                    problems.push(`${name} has more than 255 EVs in ${dex_1.Dex.stats.names[stat]}.`);
                }
            }
            if (dex.gen <= 2) {
    ...
    findEggMoveFathers(source, species, setSources, getAll, pokemonBlacklist, noRecurse) {
        ...
        if (!getAll && eggGroups.includes('Field') && !this.dex.species.get('Smeargle').isNonstandard)
            return true;
    ...
    validateForme(set) {
        ...
        if (species.name === 'Necrozma-Ultra') {
            const whichMoves = (set.moves.map(dex_1.toID).includes('sunsteelstrike') ? 1 : 0) +
                (set.moves.map(dex_1.toID).includes('moongeistbeam') ? 2 : 0);
            ...
            if (species.requiredItems && !species.requiredItems.includes(item.name)) {
                if (dex.gen >= 8 && (species.baseSpecies === 'Arceus' || species.baseSpecies === 'Silvally')) {
                    problems.push(`${name} needs to hold ${species.requiredItems.join(' or ')}.`, `(It will revert to its Normal forme if you remove the item or give it a different item.)`);
                }
                else {
                    // Memory/Drive/Griseous Orb/Plate/Z-Crystal - Forme mismatch
                    const baseSpecies = this.dex.species.get(species.changesFrom);
                    problems.push(`${name} needs to hold ${species.requiredItems.join(' or ')} to be in its ${species.forme} forme.`, `(It will revert to its ${baseSpecies.baseForme || 'base'} forme if you remove the item or give it a different item.)`);
                }
            }
            if (species.requiredMove && !set.moves.map(dex_1.toID).includes((0, dex_1.toID)(species.requiredMove))) {
                const baseSpecies = this.dex.species.get(species.changesFrom);
                problems.push(`${name} needs to know the move ${species.requiredMove} to be in its ${species.forme} forme.`, `(It will revert to its ${baseSpecies.baseForme} forme if it forgets the move.)`);
            }
        ...
        if (species.name in crowned) {
            const behemothMove = set.moves.map(dex_1.toID).indexOf(crowned[species.name]);
            if (behemothMove >= 0) {
                set.moves[behemothMove] = 'ironhead';
            }
        }
        if (species.baseSpecies === "Hoopa" && dex.gen >= 9) {
            const moves = set.moves.map(dex_1.toID);
        ...
        if (species.baseSpecies === "Rockruff" && (0, dex_1.toID)(set.ability) === 'owntempo') {
    ...
    checkSpecies(set, species, tierSpecies, setHas) {
        ...
        let nonexistentCheck = tags_1.Tags.nonexistent.genericFilter(tierSpecies) && ruleTable.check('nonexistent');
        const EXISTENCE_TAG = ['past', 'future', 'lgpe', 'unobtainable', 'cap', 'custom', 'nonexistent'];
        for (const ruleid of ruleTable.tagRules) {
            if (ruleid.startsWith('*'))
                continue;
            const tagid = ruleid.slice(12);
            const tag = tags_1.Tags[tagid];
    ...
    validateEvent(set, setSources, eventData, eventSpecies, because = ``, from = `from an event`) {
        ...
        let name = set.species;
        const species = dex.species.get(set.species);
        const maxSourceGen = this.ruleTable.has('allowtradeback') ? utils_1.Utils.clampIntRange(dex.gen + 1, 1, 8) : dex.gen;
        if (!eventSpecies)
        ...
        if ((dex.gen === 3 || dex.gen === 4) && eventData.level === 100 && set.evs) {
            ...
            for (statName in set.evs) {
                ...
                if (ev > 100) {
                    problems.push(`${name} can't have more than 100 EVs in any stat, because it is only obtainable from level 100 events. Level 100 Pokemon can only gain EVs from vitamins (Carbos etc), which are capped at 100 EVs.`);
                }
                if (!(ev % 10 === 0 ||
                    (ev % 10 === 8 && ev % 4 === 0))) {
                    problems.push(`${name} can only have EVs that are multiples of 10, because it is only obtainable from level 100 events. Level 100 Pokemon can only gain EVs from vitamins (Carbos etc), which boost in multiples of 10.`);
                }
            }
        }
        ...
        if (eventData.ivs) {
            ...
            for (statName in eventData.ivs) {
                ...
                if (set.ivs[statName] !== eventData.ivs[statName]) {
                    if (fastReturn)
                        return true;
                    problems.push(`${name} must have ${eventData.ivs[statName]} ${dex_1.Dex.stats.names[statName]} IVs${etc}.`);
                }
            }
            if (canBottleCap) {
            ...
            if (dex.gen <= 5 && eventData.abilities && eventData.abilities.length === 1 && !eventData.isHidden) {
                if (species.name === eventSpecies.name) {
                    ...
                    if (ability1.gen && eventData.generation >= ability1.gen) {
                        ...
                        const requiredAbilitySlot = ((0, dex_1.toID)(eventData.abilities[0]) === ability1.id ? 1 : 0);
    ...
    allSources(species) {
        ...
        const maxSourceGen = this.ruleTable.has('allowtradeback') ? utils_1.Utils.clampIntRange(this.dex.gen + 1, 1, 8) : this.dex.gen;
    ...
    validateMoves(species, moves, setSources, set, name = species.name, moveLegalityWhitelist = {}) {
        ...
        if (setSources.size() && setSources.moveEvoCarryCount > 3) {
            if (setSources.sourcesBefore < 6)
                setSources.sourcesBefore = 0;
            setSources.sources = setSources.sources.filter(source => source.charAt(1) === 'E' && parseInt(source.charAt(0)) >= 6);
            if (!setSources.size()) {
        ...
        if (setSources.isHidden) {
            setSources.sources = setSources.sources.filter(source => parseInt(source.charAt(0)) >= 5);
        ...
        if (setSources.babyOnly && setSources.sources.length) {
            ...
            const babyEvo = (0, dex_1.toID)(baby.evos[0]);
        ...
        for (const { species, learnset } of fullLearnset) {
            ...
            for (let learned of sources) {
                ...
                if (formeCantInherit && (learned.charAt(1) !== 'E' || learnedGen < 9))
                    continue;
                if (setSources.learnsetDomain && !setSources.learnsetDomain.includes(`${learnedGen}${(0, dex_1.toID)(species.baseSpecies)}`) &&
                ...
                if (learnedGen === 9 && learned.charAt(1) !== 'S')
                    canUseHomeRelearner = true;
                ...
                if (learned.charAt(1) === 'L') {
                    ...
                    if (level >= parseInt(learned.substr(2)) || learnedGen === 7) {
                        ...
                        if (species.gender === 'M' && !this.motherCanLearn((0, dex_1.toID)(species.mother), moveid)) {
                ...
                if (learnedGen >= 8 && learned.charAt(1) === 'E' && learned.slice(1) !== 'Eany' &&
                    ...
                    else if (learnedGen < 6 || (species.mother && !this.motherCanLearn((0, dex_1.toID)(species.mother), moveid))) {
                        limitedEggMove = move.id;
                    }
                ...
                if (learned.charAt(1) === 'E' && learnedGen >= 8 && !canLearnSpecies.includes((0, dex_1.toID)(baseSpecies.baseSpecies))) {
                    canLearnSpecies.push((0, dex_1.toID)(baseSpecies.baseSpecies));
                }
                if (!canLearnSpecies.includes((0, dex_1.toID)(species.baseSpecies)))
                    canLearnSpecies.push((0, dex_1.toID)(species.baseSpecies));
            ...
            if (canUseHomeRelearner) {
                ...
                for (const source of fullSources) {
                    ...
                    if (source.charAt(1) === 'S')
                        continue;
                    if (source.charAt(1) === 'L' && level < parseInt(source.substr(2)))
                        continue;
                    return null;
                }
            }
        ...
        if (!tradebackEligible) {
            ...
            while (nextSpecies.name) {
                for (let gen = nextSpecies.gen; gen <= dex.gen; gen++) {
                    ...
                    */
                    const baseSpeciesID = (0, dex_1.toID)(nextSpecies.baseSpecies);
    ...
    getExternalLearnsetData(species, mod) {
        const moddedDex = this.dex.mod(mod);
        if (moddedDex.species.get(species).isNonstandard)
            return null;
