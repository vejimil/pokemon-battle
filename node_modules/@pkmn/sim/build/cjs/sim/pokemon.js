...
class Pokemon {
    constructor(set, side) {
        ...
        if (this.battle.format.mod === 'gen7letsgo')
            this.happiness = 70;
    ...
    getStat(statName, unboosted, unmodified) {
        ...
        if (statName === 'spe' && stat > 10000 && !this.battle.format.battle?.trunc)
            stat = 10000;
        return stat;
    }
    getActionSpeed() {
    ...
    */
    ...
    ignoringItem(isFling = false) {
        if (this.getItem().isPrimalOrb)
            return false;
        if (this.battle.gen >= 5 && !this.isActive)
            return true;
        if (this.volatiles['embargo'] || this.battle.field.pseudoWeather['magicroom'])
            return true;
        // check Fling first to avoid infinite recursion
        if (isFling)
            return this.battle.gen >= 5 && this.hasAbility('klutz');
    ...
    getLastDamagedBy(filterOutSameSide) {
        const damagedBy = this.attackedBy.filter(attacker => (typeof attacker.damageValue === 'number' &&
    ...
    getDynamaxRequest(skipChecks) {
        ...
        if (!atLeastOne)
            return;
        if (this.canGigantamax && this.gigantamax)
    ...
    formeChange(speciesId, source = this.battle.effect, isPermanent, abilitySlot = '0', message) {
        ...
        if (isPermanent && (!source || !['disguise', 'iceface'].includes(source.id))) {
            ...
            this.baseAbility = (0, dex_1.toID)(ability);
        }
        if (this.terastallized) {
            this.knownType = true;
            this.apparentType = this.terastallized;
    ...
    updateMaxHp() {
        ...
        if (newBaseMaxHp === this.baseMaxhp)
            return;
