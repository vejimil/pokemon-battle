...
class Battle {
    constructor(options, dex = dex_1.Dex) {
    ...
    fieldEvent(eventid, targets) {
        ...
        while (handlers.length) {
            ...
            if (handler.state?.target instanceof pokemon_1.Pokemon) {
            ...
            else if (handler.state?.target instanceof side_1.Side && !handler.state.isSlotCondition) {
    ...
    singleEvent(eventid, effect, state, target, source, sourceEffect, relayVar, customCallback) {
        ...
        if (eventid === 'SwitchIn' && effect.effectType === 'Ability' && effect.flags['breakable'] &&
            this.suppressingAbility(target)) {
    ...
    findEventHandlers(target, eventName, source) {
        ...
        const shouldBubbleDown = target instanceof side_1.Side;
        ...
        if (target instanceof pokemon_1.Pokemon && (target.isActive || source?.isActive)) {
            ...
            for (const side of this.sides) {
                if (shouldBubbleDown) {
                    for (const active of side.active) {
                        ...
                        if (prefixedHandlers)
                            handlers = handlers.concat(this.findPokemonEventHandlers(active, `onAny${eventName}`));
                    }
                }
                if (side.n < 2 || !side.allySide) {
                    if (side === target || side === target.allySide) {
    ...
    skillSwap(source, target) {
        if (source.fainted || target.fainted)
            return false;
        if (source.volatiles['dynamax'] || target.volatiles['dynamax'])
            return false;
        const sourceAbility = source.getAbility();
        const targetAbility = target.getAbility();
        if (sourceAbility.flags['failskillswap'] || targetAbility.flags['failskillswap'])
            return false;
        if (this.gen <= 5 && sourceAbility.id === targetAbility.id)
            return false;
        ...
        source.abilityState = this.initEffectState({ id: (0, dex_1.toID)(source.ability), target: source });
        target.abilityState = this.initEffectState({ id: (0, dex_1.toID)(target.ability), target });
    ...
    maybeTriggerEndlessBattleClause(trappedBySide, stalenessBySide) {
        ...
        if (this.gen <= 1) {
            const noProgressPossible = this.sides.every(side => {
                ...
                const foeAllTransform = side.foe.pokemon.every(pokemon => (pokemon.fainted ||
    ...
    checkEVBalance() {
        let limitedEVs = null;
        for (const side of this.sides) {
            const sideLimitedEVs = !side.pokemon.some(pokemon => Object.values(pokemon.set.evs).reduce((a, b) => a + b, 0) > 510);
            if (limitedEVs === null) {
    ...
    spreadModify(baseStats, set) {
        const modStats = { hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
        for (const statName in baseStats) {
            modStats[statName] = this.statModify(baseStats, set, statName);
        }
        return modStats;
    }
    ...
    runAction(action) {
        ...
        const switches = this.sides.map(side => side.active.some(pokemon => pokemon && !!pokemon.switchFlag));
        for (let i = 0; i < this.sides.length; i++) {
    ...
    undoChoice(sideid) {
        ...
        side.clearChoice();
        if (updated)
            side.emitRequest(side.activeRequest, true);
    }
    /**
    ...
    hint(hint, once, side) {
        if (this.hints.has(side ? `${side.id}|${hint}` : hint))
            return;
        if (side) {
        ...
        if (once)
            this.hints.add(side ? `${side.id}|${hint}` : hint);
    }
        ...
        for (const side of this.sides) {
            const team = side.pokemon.map(pokemon => {
                ...
                if (((0, dex_1.toID)(set.species) === 'zacian' && (0, dex_1.toID)(set.item) === 'rustedsword') ||
                    ...
                    const ironHeadIndex = set.moves.map(dex_1.toID).indexOf('ironhead');
                    if (ironHeadIndex >= 0) {
